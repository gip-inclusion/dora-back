...
class Command(BaseCommand):
    ...
    def print_error(self, agency, establishment, message):
        self.stdout.write("###")
        self.stdout.write(
            self.style.ERROR(
                f'{agency["libelleEtendu"]} ({agency["code"]}) : {message}'
            )
        )
        if establishment:
            self.stdout.write(
                f'Nom SIRENE: {establishment.name}, nom API: {agency["libelleEtendu"]}'
            )
        self.stdout.write(json.dumps(agency))
        self.stdout.write("")
        ...
        for agency in agencies:
            try:
                ...
                if agency["type"] not in ("RPE", "APES"):
                    self.print_error(agency, None, "SIRET MANQUANT")
                    continue
            try:
                ...
                self.print_error(
                    agency,
                    None,
                    f"SIRET INVALIDE: https://annuaire-entreprises.data.gouv.fr/etablissement/{siret}",
                )
                continue
            ...
            commune_implantation = agency["adressePrincipale"]["communeImplantation"]
            if establishment.city_code != commune_implantation:
                self.print_error(
                    agency,
                    establishment,
                    f"Code INSEE probablement incorrect: trouvé {commune_implantation}, attendu: "
                    f"{establishment.city_code} — https://annuaire-entreprises.data.gouv.fr/etablissement/{siret}",
                )
                continue
            ...
            if not lat or not lon:
                self.print_error(
                    agency,
                    establishment,
                    f"Coordonnées géographiques manquantes: lat: {lat}, lon: {lon}",
                )
                continue
